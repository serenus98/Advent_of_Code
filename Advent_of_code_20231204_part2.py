...
cards_dict = {}
for card in cards:
    cards_dict[index_of_card(card)] = 1
for card in cards:
    starting_index = index_of_card(card)+1
    stopping_index = starting_index + points_of_card(card)
    for i in range(starting_index, stopping_index):
        cards_dict[i] += cards_dict[starting_index-1]
        #print(cards_dict)
result = 0
for key in cards_dict:
    result += cards_dict[key]
